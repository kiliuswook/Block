...
func _draw() -> void:
	draw_rect(Rect2(0, 0, 1330, 1010), Color("2a3340"))
	...
	# 잠금 실루엣.
	for r in chars.size():
		CatSprite.paint(self, Vector2(70 + r * 100, 560), 60.0, chars[r], 3, false)
	...
	for r in chars.size():
		for t in 4:
			CatSprite.paint(self, Vector2(720 + r * 100, 560 + t * 110), 60.0,
					chars[r], t, true, tints)
	# 코드 렌더와 나란히 — 같은 크기·중심이 맞는지.
	for t in 4:
		Player.paint_cat(self, Vector2(700, 80 + t * 110), 60.0, 0.0, true, false,
				GameState.cat_skin("cream"))
	var f := ThemeDB.fallback_font
	draw_string(f, Vector2(660, 30), "코드렌더", HORIZONTAL_ALIGNMENT_LEFT, -1, 18, Color.WHITE)
	draw_string(f, Vector2(20, 520), "잠금 실루엣 (윗줄) / 파츠 리컬러 (오른쪽 6종 × 4단계)",
