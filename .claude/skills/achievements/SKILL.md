---
name: achievements
description: 업적 작업 규칙 — Achv autoload 구조, 업적 추가·판정 절차, 세이브 필드 규칙, 스팀 파트너 사이트 등록, 모바일 전용 업적 화면 방침, 개발 완료 시점 최종 검토 체크리스트. 업적을 추가·수정·검토하거나 스팀 앱 id를 받아 실제 업적을 붙일 때 참조.
---

# 업적 작업 규칙

판정은 한 벌(`Achv`), 표시는 플랫폼마다 다르다 — **스팀 빌드에는 게임 안 업적 목록 UI를
만들지 않고**(오버레이가 대신한다), **모바일 빌드에는 반드시 만든다**(오버레이가 없다).
...
- 스팀 쪽만 보면 `Achv.DEFS`에 **id와 조건 메모만** 있으면 된다. 표시 문자열을 판정 코드에 섞지 말 것.

## 📱 모바일 업적 화면 (2026-08-31 구현)

스팀은 오버레이가 표시를 대신하지만 모바일에는 그런 게 없다 — 목록이 없으면 유저가 딴 업적을
볼 방법이 아예 없다. 그래서 **모바일 타이틀 메뉴에 `업적` 항목을 따로 둔다.**

| | |
|---|---|
| 화면 | `mobile/ui/achievements_panel.gd` (class_name 없음 — `title_mobile.gd`가 preload) |
| 진입 | 모바일 타이틀 메뉴의 `업적` 카드 — `title.gd`의 훅 `extra_menu_cards()`를 `title_mobile.gd`가 오버라이드해 한 장 더 붙인다 (core는 `mobile/`을 모른다) |
| 문구 | `MENU_ACHV` · `ACHV_TITLE` · `ACHV_DONE_COUNT` · `ACHV_DONE` / `ACHV_LOCKED` · **`ACHV_<ID>_NAME` / `_DESC`** (`shared/locale/ui.csv`, 지금은 en·ko) |
| 진행률 | `Achv.progress(id) -> Vector2i` (지금, 목표) — 목표 0이면 셀 수 없는 것(사건형) |
| 스크린샷 | `.tmp_shots/m_title_achv.png` (`tests/visual_capture.gd`) |

- **판정·해금 기록은 그대로 `Achv`/`GameState.achv`가 소유한다.** 화면은 읽기만 한다 —
  업적 화면 때문에 `Achv`에 **판정** 로직을 늘리지 말 것. `progress()`는 판정이 아니라
  "보여 주기 위한 셈"이고, `check()`는 이 값을 쓰지 않는다.
- 업적을 추가하면 **번역 두 줄(`_NAME`/`_DESC`)을 같이 넣는다** — 없으면 화면에 키가 그대로 나온다.
  (스팀만 보고 넘어가기 쉬운 함정이다. `docs/steam_setup.md` ③ 표 + CSV 두 줄이 한 세트.)
- 목록은 `Achv.DEFS` 순서 그대로 나온다 — 순서가 곧 화면 순서다.
- 아이콘 30장(해금·잠금 × 15)은 스팀 파트너 사이트용과 **같은 걸 공유**할 것. 지금 화면은
  아이콘 없이 금색 트로피/자물쇠 뱃지로 그린다 — 아트가 나오면 뱃지 자리를 그림으로 바꾼다.
- 해금 순간의 토스트는 아직 없다 (스팀은 오버레이가 띄운다).
- Play Games / Game Center 연동은 별개다. 하게 되면 `mobile/mobile_platform.gd`가
  `unlock_achievement()`를 오버라이드하는 것으로 끝나고, 이 화면은 그대로 둔다.
...
- [ ] 아이콘 30장(해금·잠금 × 15) 준비 — 각 256×256 png (모바일 업적 화면과 공유)
- [ ] **모바일 업적 화면**의 `ACHV_*_NAME`/`_DESC`가 13개국어로 채워졌는지 (지금 en·ko)
