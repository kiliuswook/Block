...
- 화면 분할 2인(`GameState.split`): 인원 선택에서 2인을 고르면 켜진다. 탈출 경주/무한을 SubViewport 2개로 나눠 경쟁(라운드제 3선승). 분할 중 스토리 스테이지 로직은 비활성(항상 문 열린 탈출 경주). **스테이지 모드 2인은 라운드제가 아니라 "1인과 같은 기록을 위한 한 판 승부"다**(`main.gd._on_stage_split_finished()`/`_finish_stage_split()`): 양쪽이 각자 우물에서 레벨을 올리다가 먼저 죽은 좌석은 **그 상태로 멈춘 채** 남고 남은 좌석은 자기 판이 끝날 때까지 계속 논다. 둘 다 끝나면 점수(동점이면 LEVEL → 지운 줄) 높은 쪽이 승자고, **승자의 성적만 1인 플레이와 똑같은 경로로 기록된다** — `record_weekly`/`record_classic`/`Ranks.submit("classic")`·`classic_level_best`·`Achv.check()`. 골드·계정 경험치·리플레이는 분할이라 여전히 안 준다. 그래서 점수는 좌석마다 따로 세야 한다: `escape_board.gd`의 모든 가산이 `_add_score()`를 지나 보드 자기 몫 `run_score`에 쌓이고, **분할이 아닐 때만** 공용 `GameState.score`에도 반영된다. 아케이드 HUD(LEVEL/LINES 랙·다음 레벨 버튼)는 분할에서 아예 만들지 않고(`main.gd._ready`의 `classic and not GameState.split`), 진행은 좌석 라벨 `SPLIT_STAGE`/`SPLIT_STAGE_OVER`와 가운데 점수 대결(`_update_versus_tally()`)로 읽는다. 대응해 `escape_board.gd`의 classic EventBus 알림도 `split`이면 보내지 않는다. 회귀 테스트: `res://tests/test_split_stage.tscn` → `PASS`. 보드 씬은 `core/scenes/board.tscn`, 분할 빌드는 `main.gd._build_split()`. 좌석 배치 = 키보드 배치: P1(왼쪽 화면)이 WASD+Q/E+Ctrl(`p2_*` 액션), P2(오른쪽 화면)가 방향키+, .(또는 Z/X)+Shift(기본 액션). 보드/플레이어의 `act_*` 변수로 액션 이름 주입. 분할 중 보드는 EventBus 대신 로컬 `finished(win)` 시그널 사용
